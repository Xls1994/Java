...
public class BiLSTMTargetConnectMain {
	...
	public BiLSTMTargetConnectMain(
	...
	{ 
		...
		seedLSTMCellForw = new SimplifiedLSTMLayer(embeddingLength * 2);
		seedLSTMCellForw.randomize(rnd, -1.0 * randomizeBase, randomizeBase);
		
		seedLSTMCellBack = new SimplifiedLSTMLayer(embeddingLength * 2);
		seedLSTMCellBack.randomize(rnd, -1.0 * randomizeBase, randomizeBase);
		
		connect = new MultiConnectLayer(new int[]{embeddingLength * 2, embeddingLength * 2});
		
		linearForSoftmax = new LinearLayer(embeddingLength * 4, classNum);
			...
			) throws Exception
	{
		...
		for(int round = 1; round <= roundNum; round++)
		{
			...
			for(int idxData = 0; idxData < trainDataList.size(); idxData++)
			{
				...
				SentenceLSTMTargetConnect sentLSTMForw = new SentenceLSTMTargetConnect(
						forwWordIds,
						seedLookup,
						seedLSTMCellForw,
						targetVec);
				
				SentenceLSTMTargetConnect sentLSTMBack = new SentenceLSTMTargetConnect(
						backWordIds,
						seedLookup,
						seedLSTMCellBack,
						targetVec);
				...
				sentLSTMForw.update(learningRate);
				sentLSTMBack.update(learningRate);
...
//	
	public void predict(int round, String dumpResultFile) throws Exception
	{
	...
	public static void main(String[] args) throws Exception
	{
		...
		BiLSTMTargetConnectMain main = new BiLSTMTargetConnectMain(
				embeddingFile, 
				embeddingLength, 
				classNum, 
				trainFile, 
