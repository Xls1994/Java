...
public class SentenceEntityMemNN_Position_3 implements NNInterface{
	...
	public SentenceEntityMemNN_Position_3()
	{
...
//			tmpLookup.link(lookupConnect, i);
	...
	public void forward() {
		...
		for(int i = 0; i < lookupList.size(); i++)
		{
			for(int j = 0; j < lookupList.get(i).output.length; j++)
				// we also tried multiplication, however, it performs very bad , e.g. 50+ in terms of accuracy in laptop.
				// i will try positionembedding -> sigmoid, and use multiplication like a neural gate in lstm.
				lookupConnect.input[i][j] = lookupList.get(i).output[j] + positionLookupList.get(i).output[j]; 
		}
	...
	public void backward() {
		...
		for(int i = 0; i < positionLookupList.size(); i++)
		{
			for(int j = 0; j < positionLookupList.get(i).output.length; j++)
				positionLookupList.get(i).outputG[j] += lookupConnect.inputG[i][j];
	...
	public void updateAdaGrad(double learningRate, int batchsize) {
		...
		for(int i = 0; i < positionLookupList.size(); i++)
		{
			positionLookupList.get(i).updateAdaGrad(learningRate, batchsize);
		}
	}
